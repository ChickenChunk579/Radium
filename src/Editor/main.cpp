...
void imguiTheme() {
  ...
  style.Colors[ImGuiCol_TabUnfocusedActive] =
      ImVec4(0.65f, 0.20f, 0.25f, 1.00f);
  style.Colors[ImGuiCol_DockingPreview] = ImVec4(0.92f, 0.18f, 0.29f, 0.50f);
  style.Colors[ImGuiCol_Border] =
      ImVec4(0.92f, 0.18f, 0.29f, 1.0f); // Bright red outline, fully opaque
}
...
TextEditor::Palette RadiumPalette() {
  ...
  palette[(int)PI::Default] =
      IM_COL32(220, 220, 220, 255); // neutral light gray
  palette[(int)PI::Keyword] =
      IM_COL32(235, 46, 71, 255); // red (from theme: 0.92f, 0.18f, 0.29f)
  ...
  palette[(int)PI::PreprocIdentifier] =
      IM_COL32(255, 109, 122, 255);                         // soft red-pink
  palette[(int)PI::Comment] = IM_COL32(144, 153, 163, 255); // gray-blue
  palette[(int)PI::MultiLineComment] =
      IM_COL32(144, 153, 163, 255); // gray-blue
  palette[(int)PI::Background] =
      IM_COL32(34, 36, 44, 255); // dark background (0.13f, 0.14f, 0.17f)
  palette[(int)PI::Cursor] = IM_COL32(255, 255, 255, 255); // white
  palette[(int)PI::Selection] =
      IM_COL32(235, 46, 71, 110); // red + alpha (ImGuiCol_TextSelectedBg)
  ...
  palette[(int)PI::CurrentLineFill] =
      IM_COL32(64, 64, 64, 0); // subtle background
  palette[(int)PI::CurrentLineFillInactive] =
      IM_COL32(48, 48, 48, 0); // slightly dimmer
  palette[(int)PI::CurrentLineEdge] =
      IM_COL32(60, 60, 60, 0); // line edge indicator
...
public:
  ...
  std::string GetTitle() override { return "Radium Editor"; }

  Radium::Vector2i GetPreferredSize() override { return {1280, 720}; }

  Radium::Vector2f GetGravity() override { return {0.0f, -200.0f}; }

  void OnLoad() override {
    scene = new Radium::SubViewport(1280, 720);
    // REENABLE ME IF IT FAILS
    Rune::Texture *tex = new Rune::Texture(scene->viewport->textureView,
                                           Rune::SamplingMode::Nearest);
  ...
  void OnTick(float dt) override {
    ...
    if (Radium::Input::IsMiddleMouseButtonDown()) {
      if (!isDraggingCamera) {
        ...
        // Update camera offset (negative because we're moving the world, not
        // the camera)
      ...
      if (cameraVelocity.x != 0.0f || cameraVelocity.y != 0.0f) {
        Radium::currentApplication->GetCamera()->offset.x -=
            cameraVelocity.x * dt;
        Radium::currentApplication->GetCamera()->offset.y -=
            -cameraVelocity.y * dt;
        ...
        if (std::abs(cameraVelocity.x) < 0.1f)
          cameraVelocity.x = 0.0f;
        if (std::abs(cameraVelocity.y) < 0.1f)
          cameraVelocity.y = 0.0f;
      }
    }
  }

  void OnPreRender() override { scene->OnRender(); }

  void OnRender() override {}

  bool IsDescendantOf(Radium::Nodes::Node *parent,
                      Radium::Nodes::Node *possibleChild) {
    if (!possibleChild)
      return false;
  ...
  void DrawNode(Radium::Nodes::Node *node) {
    ...
    bool open = ImGui::TreeNodeEx((void *)(intptr_t)node, flags, "%s",
                                  node->name.c_str());
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *payload =
              ImGui::AcceptDragDropPayload("RADIUM_NODE")) {
        Radium::Nodes::Node *dragged = *(Radium::Nodes::Node **)payload->Data;

        if (dragged != node && dragged->parent != node &&
            !IsDescendantOf(dragged, node)) {
          // Detach from old parent
          if (dragged->parent) {
            auto &siblings = dragged->parent->children;
            siblings.erase(
                std::remove(siblings.begin(), siblings.end(), dragged),
                siblings.end());
          }
          ...
          roots.erase(std::remove(roots.begin(), roots.end(), dragged),
                      roots.end());
        }
      }
    ...
    // Only draw children and TreePop if this node was actually opened (and not
    // a leaf)
    if (open && !isLeaf) {
  ...
  void OnImgui() override {
    ...
    if (ImGuiFileDialog::Instance()->Display(
      ...
      if (ImGuiFileDialog::Instance()->IsOk()) { // action if OK
        ...
        case 3: // add script
        {
          ...
          selectedNode->script = new Radium::Nodes::LuaScript(
              relativePath.generic_string(), &scene->tree, false);
        ...
        case 4: // open project
        {
          ...
          for (auto batchInfo : config.spriteBatches) {
            Radium::SpriteBatchRegistry::Add(batchInfo.tag, batchInfo.path,
                                             batchInfo.origin,
                                             Rune::SamplingMode::Nearest);
          }
    ...
    }
    {
      ...
      if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("File")) {
          if (ImGui::MenuItem("New", "Ctrl+N", false)) {
            ...
            ImGuiFileDialog::Instance()->OpenDialog(
                "ChooseFileDlgKey", "Choose Path", ".json", config);
          }
          if (ImGui::MenuItem("Open", "Ctrl+O", false)) {
            ...
            config.path = ".";
            ImGuiFileDialog::Instance()->OpenDialog(
                "ChooseFileDlgKey", "Open JSON Scene", ".json", config);
          }
          if (ImGui::MenuItem("Save", "Ctrl+S", false)) {
            if (openPath == "") {
              ...
              config.path = ".";
              ImGuiFileDialog::Instance()->OpenDialog(
                  "ChooseFileDlgKey", "Save JSON Scene", ".json", config);
            } else {
              Flux::Info("Open path {}", openPath);
              scene->tree.Serialize(
                  (fs::path(projectFolder) / openPath).generic_string());
            }
          }
        ...
        if (ImGui::BeginMenu("Project")) {
          if (ImGui::MenuItem("Open", "Ctrl+Shift+O", false)) {
            ...
            config.path = ".";
            ImGuiFileDialog::Instance()->OpenDialog(
                "ChooseFileDlgKey", "Select app.json file", ".json", config);
          }
          if (ImGui::MenuItem("Run", "Ctrl+R", false)) {
            TinyProcessLib::Process lsProcess(
                std::filesystem::current_path().string() + "/RadiumRuntime .",
                projectFolder, [](const char *bytes, size_t n) {
                  std::cout << std::string(bytes, n);
                });
          }
          if (ImGui::MenuItem("Refresh Batches", NULL, false)) {
            std::string appConfig =
                Radium::ReadFileToString(projectFolder + "/app.json");
            json j = json::parse(appConfig);
            auto config = j.get<Radium::GameConfig>();
      ...
      // Add this OUTSIDE the menu bar (after ImGui::End() of the dockspace
      // window) Check for keyboard shortcuts
    ...
    }
    {
      ...
      if (selectedNode) {
        ...
        if (selectedNode->script) {
          Radium::Nodes::LuaScript *script =
              reinterpret_cast<Radium::Nodes::LuaScript *>(
                  selectedNode->script);
          if (script != nullptr)
        ...
        if (ImGui::Button("Select a new Lua script")) {
          ...
          ImGuiFileDialog::Instance()->OpenDialog(
              "ChooseFileDlgKey", "Choose a lua script", ".lua", config);
        }

        ImGui::Separator();

        for (auto property :
             Radium::Nodes::ClassDB::GetProperties(selectedNode)) {
          ...
          if (Radium::Nodes::ClassDB::IsEnum(property.type)) {
            int *f = Radium::Nodes::ClassDB::GetPropertyPointer<int>(
                property.name, selectedNode);
            ImGui::InputInt(property.name.c_str(), f);
          }
          if (property.type == "Radium::Vector2f") {
            Radium::Vector2f *value =
                Radium::Nodes::ClassDB::GetPropertyPointer<Radium::Vector2f>(
                    property.name, selectedNode);
          ...
          if (property.type ==
              ...
              "std::allocator<char> >") {
#endif
            // Edit the actual property string in-place. Use GetPropertyPointer
            // so changes persist back to the node.
            std::string *strPtr =
                Radium::Nodes::ClassDB::GetPropertyPointer<std::string>(
                    property.name, selectedNode);
            if (strPtr) {
          ...
          if (property.type == "float") {
            float *f = Radium::Nodes::ClassDB::GetPropertyPointer<float>(
                property.name, selectedNode);
            ImGui::InputFloat(property.name.c_str(), f);
          }
          if (property.type == "unsigned int") {
            unsigned int *f =
                Radium::Nodes::ClassDB::GetPropertyPointer<unsigned int>(
                    property.name, selectedNode);
          ...
          if (property.type == "bool") {
            bool *f = Radium::Nodes::ClassDB::GetPropertyPointer<bool>(
                property.name, selectedNode);
            ImGui::Checkbox(property.name.c_str(), f);
          }
          if (property.type == "Radium::RectangleF") {
            Radium::RectangleF *value =
                Radium::Nodes::ClassDB::GetPropertyPointer<Radium::RectangleF>(
                    property.name, selectedNode);
          ...
          if (property.type == "Radium::Nodes::Node*") {
            Radium::Nodes::Node **ptr =
                Radium::Nodes::ClassDB::GetPropertyPointer<
                    Radium::Nodes::Node *>(property.name, selectedNode);

            if (ptr && *ptr)
              ImGui::Text("%s: %s", property.name.c_str(),
                          (*ptr)->name.c_str());
            else
            ...
            if (ImGui::BeginDragDropTarget()) {
              if (const ImGuiPayload *payload =
                ...
                Radium::Nodes::Node *dropped =
                    *(Radium::Nodes::Node **)payload->Data;
...

    {
      ...
      std::string expandedFolder =
          (fs::path(projectFolder) / folder).generic_string();
    ...
    }
    {
      ...
      if (selectedNode) {
        if (ImGui::Button("Copy")) {
          ...
          copiedNodeJson = Radium::Nodes::SerializeNode(
              selectedNode); // Assumes you have SerializeToString()
        ...
        if (ImGui::Button("Paste")) {
          if (!copiedNodeJson.empty()) {
            ...
            Radium::Nodes::Node *cloned = Radium::Nodes::DeserializeNode(
                copiedNodeJson, &scene->tree, nullptr,
                true); // You’ll implement this
            if (cloned) {
              cloned->name += "_copy";
        ...
        if (ImGui::Button("Up")) {
          auto &siblings =
              (selectedNode->parent ? selectedNode->parent->children
                                    : scene->tree.nodes);
        ...
        if (ImGui::Button("Delete")) {
          auto it = std::find(scene->tree.nodes.begin(),
                              scene->tree.nodes.end(), selectedNode);
      ...
      static std::vector<std::string> items =
          Radium::Nodes::ClassDB::GetNodeClasses();
      ...
      if (selectedItem != -1) {
        ...
        auto node =
            (Radium::Nodes::Node *)Radium::Nodes::ClassDB::Create(selected);
    ...
    }
    {
      ...
      int imgW =
          (scene && scene->viewport->width > 0) ? scene->viewport->width : 1280;
      int imgH = (scene && scene->viewport->height > 0)
                     ? scene->viewport->height
                     : 720;

      float imageAspect = (float)imgW / (float)imgH;

      // Compute render size that fits the available region while keeping the
      // aspect ratio
      ImVec2 renderSize;
      ...
      ImGui::SetCursorScreenPos(
          ImVec2(cursorScreenPos.x + offsetX, cursorScreenPos.y + offsetY));
      cursorScreenPos = ImGui::GetCursorScreenPos();

      ImGui::Image(sceneTexture, renderSize);

      // Move cursor to the end of the content region so following widgets are
      // laid out correctly
      ImGui::SetCursorScreenPos(
          ImVec2(cursorScreenPos.x + avail.x, cursorScreenPos.y + avail.y));
